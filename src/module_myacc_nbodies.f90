...
subroutine move_data_to_device_acc(NB)
    ...
    !$acc enter data copyin(NB)
    
    ! Now copy all the allocatable arrays within the structure
    ! Arrays that are initialized on host
    !$acc enter data copyin(NB%pos(1:N,1:nd))
    !$acc enter data copyin(NB%pos_0(1:N,1:nd))
    !$acc enter data copyin(NB%vel(1:N,1:nd))
    !$acc enter data copyin(NB%vel_0(1:N,1:nd))
    !$acc enter data copyin(NB%mass(1:N))
    !$acc enter data copyin(NB%mass_inv(1:N))
    !$acc enter data copyin(NB%radius(1:N))
    
    ! Arrays that will be computed on device - create them
    !$acc enter data create(NB%vel_h(1:N,1:nd))
    !$acc enter data create(NB%accel(1:N,1:nd))
    !$acc enter data create(NB%force(1:N,1:nd))
    !$acc enter data create(NB%bc_factor(1:N,1:nd))
    !$acc enter data create(NB%kinetic_energy(1:N))
    !$acc enter data create(NB%potential_energy(1:N))
    !$acc enter data create(NB%sum_energy(1:N))
    !$acc enter data create(NB%center_of_mass(1:nd))
    !$acc enter data create(NB%center_of_mass_velocity(1:nd))
    
    ! ~~~~~~~~~~~~~~~~~~~~~ OTHER GLOBAL VARIABLES ~~~~~~~~~~~~~~~~~~~~~~
    ! Copy other module variables
    !$acc enter data copyin(ndim) 
    !$acc enter data copyin(Gravitational_Constant)
    !$acc enter data copyin(Softening_Length, Softening_Length_Squared)
    ! small_number is a parameter - don't copy it
    !$acc enter data copyin(dt, dt_half)
    !$acc enter data copyin(REFLECTIVE_BC)
    !$acc enter data copyin(iReflective_BC(1:3))
    !$acc enter data copyin(L_bound(1:3))
    ! Create temporary variables for reductions
    !$acc enter data create(total_KE, total_PE)
  ...
  subroutine update_host_for_dump_acc(NB)
    ...
    !$acc update host(NB%pos(1:N,1:nd))
    !$acc update host(NB%vel(1:N,1:nd))
    !$acc update host(NB%accel(1:N,1:nd))
    !$acc update host(NB%force(1:N,1:nd))
    !$acc update host(NB%kinetic_energy(1:N))
    !$acc update host(NB%potential_energy(1:N))
    !$acc update host(NB%sum_energy(1:N))
  ...
  subroutine update_host_for_diagnostics_acc(NB)
    ...
    !$acc update host(NB%total_kinetic_energy)
    !$acc update host(NB%total_potential_energy)
    !$acc update host(NB%total_energy)
    !$acc update host(NB%center_of_mass(1:nd))
    !$acc update host(NB%center_of_mass_velocity(1:nd)) 
    !$acc update host(NB%total_energy_initial) 
  ...
  subroutine cleanup_device_acc(NB)
    ...
    !$acc exit data delete(NB%force(1:N,1:nd))
  ...
  subroutine get_global_metrics_acc(NB)
    ...
    ! Initialize totals
    !$acc kernels present(NB%total_potential_energy, NB%total_kinetic_energy, NB%total_energy, NB%center_of_mass, NB%center_of_mass_velocity, ndim,total_KE, total_PE)
...
!$acc parallel present(NB%kinetic_energy, NB%potential_energy, NB%sum_energy, NB%vel, NB%mass, &
    !$acc&                 NB%N_bodies, ndim, NB%total_kinetic_energy, NB%total_potential_energy, &
    !$acc&                 NB%center_of_mass, NB%pos, NB%total_mass_inv, NB%center_of_mass_velocity)
    ...
    do i = 1, NB%N_bodies
      do nd = 1, ndim
        !$acc atomic update
        NB%center_of_mass(nd) = NB%center_of_mass(nd) + &
    ...
    do i = 1, NB%N_bodies
      do nd = 1, ndim
        !$acc atomic update
        NB%center_of_mass_velocity(nd) = NB%center_of_mass_velocity(nd) + &
    ...
    !$acc kernels present(NB%total_energy, NB%total_kinetic_energy, NB%total_potential_energy, total_KE, total_PE)
...
!$acc parallel present(NB%force, NB%accel, NB%mass_inv, NB%N_bodies, ndim, &
