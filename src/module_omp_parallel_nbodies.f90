...
contains
  ...
  subroutine get_global_metrics_omp(NB)
    ...
    real(kind=wp) :: total_ke, total_pe
  ...
  subroutine update_positions_omp(NB)
    ...
    !$omp parallel do default(shared) private(i)
    do i = 1, NB%n_bodies
      NB%pos_00(i, 1:ndim) = NB%pos_0(i, 1:ndim)
      NB%pos_0(i, 1:ndim) = NB%pos(i, 1:ndim)
      NB%vel_00(i, 1:ndim) = NB%vel_0(i, 1:ndim)
      NB%vel_0(i, 1:ndim) = NB%vel(i, 1:ndim)
      NB%accel_00(i, 1:ndim) = NB%accel_0(i, 1:ndim)
      NB%accel_0(i, 1:ndim) = NB%accel(i, 1:ndim)
    end do
    !$omp end parallel do
